...
test.describe('Product Detail Page Visual & Component Audit', () => {

  test('Audit PDP components in Desktop and Mobile views', async ({ browser }) => {
    test.setTimeout(180000); // 180 seconds timeout
    const mode = process.env.MODE || 'compare';
    console.log(`Running PDP Component Audit in ${mode.toUpperCase()} mode...`);
    ...
    const pageConfig = config.pages.pdp;
    if (!pageConfig) {
      throw new Error("PDP page configuration not found in config.js");
    }

    // 1. Audit Desktop View
    console.log('\n--- Auditing PDP Desktop View ---');
    ...
    await runAuditForView(desktopPage, pageConfig, 'pdp_desktop', 'Product Detail Page (Desktop)', baseline, mode, runData);
    await desktopPage.close();
    await desktopContext.close();

    // 2. Audit Mobile View
    console.log('\n--- Auditing PDP Mobile View ---');
    ...
    await runAuditForView(mobilePage, pageConfig, 'pdp_mobile', 'Product Detail Page (Mobile)', baseline, mode, runData);
    ...
    if (mode === 'capture') {
      ...
      if (!isComplete) {
        console.log(`\n⚠️ Warning: PDP Baseline capture was INCOMPLETE. Missing critical components: ${missingCritical.join(', ')}`);
        throw new Error(`PDP Baseline capture failed due to missing critical components: ${missingCritical.join(', ')}`);
      } else {
        // Merge with existing baseline to preserve Home Page baseline
        let existingBaseline = {};
        if (fs.existsSync(BASELINE_PATH)) {
          try {
            existingBaseline = JSON.parse(fs.readFileSync(BASELINE_PATH, 'utf8'));
          } catch (e) {
            console.error("Could not parse existing baseline, overwriting...", e);
          }
        }
        ...
        console.log(`\n✅ PDP Baseline details merged into: ${BASELINE_PATH}`);
      }
    } else {
      // Compare Mode - Generate report dashboard (Reporter will consolidate all pages present in runData.pages)
      // Since reporter takes all pages, if we run tests separately, we'd want to load the existing baseline pages 
      // or whatever was audited in this run. To show the dashboard, let's load other run results if they exist, 
      // or write the ones from this run. Let's merge runData.pages with any cached current run data if available,
      // or just generate dashboard with the pages from this run and baseline.
      const reportPath = path.join(REPORTS_DIR, 'dashboard.html');
      
      // Let's load the previous runs' data if available, to show the complete dashboard.
