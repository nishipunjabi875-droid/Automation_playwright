...
test.describe('Category Page Visual & Component Audit', () => {

  test('Audit Category Page components in Desktop and Mobile views', async ({ browser }) => {
    test.setTimeout(180000); // 180 seconds timeout
    const mode = process.env.MODE || 'compare';
    console.log(`Running Category Page Component Audit in ${mode.toUpperCase()} mode...`);
    ...
    const pageConfig = config.pages.category;
    if (!pageConfig) {
      throw new Error("Category Page configuration not found in config.js");
    }

    // 1. Audit Desktop View
    console.log('\n--- Auditing Category Page Desktop View ---');
    ...
    await runAuditForView(desktopPage, pageConfig, 'category_desktop', 'Category Page (Desktop)', baseline, mode, runData);
    await desktopPage.close();
    await desktopContext.close();

    // 2. Audit Mobile View
    console.log('\n--- Auditing Category Page Mobile View ---');
    ...
    await runAuditForView(mobilePage, pageConfig, 'category_mobile', 'Category Page (Mobile)', baseline, mode, runData);
    ...
    if (mode === 'capture') {
      ...
      if (!isComplete) {
        console.log(`\n⚠️ Warning: Category Baseline capture was INCOMPLETE. Missing critical components: ${missingCritical.join(', ')}`);
        throw new Error(`Category Baseline capture failed due to missing critical components: ${missingCritical.join(', ')}`);
      } else {
        // Merge with existing baseline to preserve other page baselines (like Home/PDP)
        ...
        console.log(`\n✅ Category Baseline details merged into: ${BASELINE_PATH}`);
      }
    } else {
      // Compare Mode - Generate report dashboard
      const reportPath = path.join(REPORTS_DIR, 'dashboard.html');
      
      // Load previous runs' data if available to show the complete dashboard.
